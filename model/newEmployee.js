...
module.exports = (name, birth, sex, hometown, body, department, callback) => {
  ...
  mysql.query(sql_1, sqlParams_1, function (err, result) {
    if (err) {
  ...
  mysql.query(sql_2, sqlParams_2, function (err, result) {
    if (err) {
      console.log('[INSERT ERROR] - ', err.message);
      callback(false);
    }
  ...
  mysql.query(sql_3, sqlParams_2, function (err, result) {
    if (err) {
      console.log('[INSERT ERROR] - ', err.message);
      callback(false);
    }
  ...
  mysql.query(sql_4, sqlParams_4, function (err, result) {
    if (err) {
      console.log('[INSERT ERROR] - ', err.message);
      callback(false);
    }
